...
ngx_module_t ngx_http_auth_jwt_module = {
    ...
    NULL,                           /* init master */
    NULL,                           /* init module */
    ngx_http_auth_jwt_init_process, /* init process */
    NULL,                           /* init thread */
    NULL,                           /* exit thread */
                                          ...
                                          ngx_command_t *cmd, void *conf)
{
  ...
  if (cf->args->nelts > 2)
  {
    ...
    if (value[2].len <= starts_with_len || ngx_strncmp(value[2].data, starts_with, starts_with_len) != 0)
    {
...
static ngx_int_t
ngx_http_auth_jwt_init_process(ngx_cycle_t *cycle)
{
  if (ssl_global_init() != 0)
  {
    return NGX_ERROR;
  }
...
ngx_http_auth_jwt_handler(ngx_http_request_t *r, ngx_int_t phase)
{
  ...
  else if (r->headers_in.authorization && ngx_strncmp(r->headers_in.authorization->value.data,
  ...
  /* parse jwt token */
  if (jwt_parse(&ctx->jwt, (char *)ctx->token, &ctx->payload_len) != 0 || ctx->jwt == NULL)
  {
    ngx_log_error(NGX_LOG_INFO, r->connection->log, 0,
